...
export default function MatchForm({ match, tournament }: { match: any, tournament: any }) {
  ...
  const initialSets = Array.from({ length: maxSets }).map((_, i) => {
    if (match.setScores && Array.isArray(match.setScores) && match.setScores[i]) {
      return { home: String(match.setScores[i].home ?? ''), away: String(match.setScores[i].away ?? '') };
    }
    return { home: '', away: '' };
  });
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isVolleyball) {
      ...
      formData.set('setScores', JSON.stringify(validSets.map(s => ({ home: Number(s.home), away: Number(s.away) }))))
    ...
    startTransition(async () => {
      try {
        ...
        toast.error("Errore nel salvataggio", { description: err?.message || "Riprova tra poco." })
      }
    })
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between w-full md:w-auto flex-grow max-w-2xl mx-auto mb-4 md:mb-0">
        <div className="text-right font-bold text-slate-700 w-1/4 truncate pr-2" title={match.homeTeam?.name}>
          {match.homeTeam?.name}
        </div>
          ...
          <div className="flex flex-col items-center justify-center gap-2">
            <div className="flex gap-2 text-xs font-semibold text-slate-400">
              {Array.from({ length: maxSets }).map((_, i) => (
                <div key={i} className="w-10 text-center">S{i + 1}</div>
              ))}
            </div>
        ...
        <div className="text-left font-bold text-slate-700 w-1/4 truncate pl-2" title={match.awayTeam?.name}>
          {match.awayTeam?.name}
        </div>
      </div>
      ...
      <button
        type="submit"
        disabled={isPending}
        className={`px-6 py-2.5 rounded-lg font-bold shadow-sm transition w-full md:w-auto mt-4 md:mt-0 flex items-center justify-center gap-2 min-w-[90px] ${
          match.status === 'FINISHED'
