...
export const CreateTournamentSchema = z.object({
  name: z.string().min(1, "Il nome del torneo è obbligatorio").max(100, "Nome troppo lungo"),
  description: z.string().optional(),
  format: z.enum(["ROUND_ROBIN", "SINGLE_ELIMINATION", "DOUBLE_ELIMINATION", "CHAMPIONS_LEAGUE"]),
  sport: z.enum(["FOOTBALL", "BASKETBALL", "VOLLEYBALL", "TENNIS", "PADEL", "CUSTOM"]),
...
export const UpdateScoreSchema = z.object({
  ...
  setScores: z.string().optional().transform((val) => {
    if (!val) return null;
    try {
