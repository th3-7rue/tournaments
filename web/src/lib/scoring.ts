...
export type Sport = 'VOLLEYBALL' | 'FOOTBALL' | 'BASKETBALL' | 'TENNIS' | 'PADEL' | 'CUSTOM'
 ...
 */
export function calculateSetsWon(completedSets: SetScore[]): { home: number; away: number } {
 ...
 */
export function isVolleyballMatchOver(completedSets: SetScore[], maxSets: number): boolean {
 ...
 */
export function shouldEndTennisSet(homeGames: number, awayGames: number): boolean {
...
export interface ScorerConfig {
  ...
  shouldEndSet: (home: number, away: number, setNumber: number, maxSets: number) => boolean
...
export const SPORT_CONFIGS: Record<Sport, ScorerConfig> = {
  ...
  TENNIS: {
    ...
    isMatchOver: (completedSets, maxSets) => isVolleyballMatchOver(completedSets, maxSets),
  },
  PADEL: {
    hasSets: true,
    setTarget: () => 6,
