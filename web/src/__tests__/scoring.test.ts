...
describe("scoring.ts", () => {
  ...
  describe("beach volley support", () => {
    it("dovrebbe trattare beach volley come volley", () => {
      expect(isVolleyballSport("BEACH_VOLLEY")).toBe(true);
      expect(SPORT_CONFIGS.BEACH_VOLLEY.hasSets).toBe(true);
      expect(SPORT_CONFIGS.BEACH_VOLLEY.setsToWin(5)).toBe(3);
    });
  });
});
