...
describe("actions.ts", () => {
  ...
  describe("recalculateStandings", () => {
    ...
    it("dovrebbe trattare beach volley come pallavolo nelle classifiche", async () => {
      ...
      await recalculateStandings(tournamentId, "BEACH_VOLLEY");

      expect(prisma.standing.update).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: "s1" },
          data: expect.objectContaining({ points: 2, wins: 1 }),
        }),
      );
      expect(prisma.standing.update).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: "s2" },
          data: expect.objectContaining({ points: 1, losses: 1 }),
        }),
      );
    });
  });
